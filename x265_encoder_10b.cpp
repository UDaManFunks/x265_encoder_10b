...
const uint8_t X265Encoder10b::s_UUID[] = { 0x2b, 0x88, 0xe8, 0x41, 0xd8, 0xe4, 0x40, 0x4b, 0x87, 0x9e, 0xa4, 0x80, 0xfc, 0x90, 0xda, 0xb3 };
	...
	StatusCode Render(HostListRef* p_pSettingsList)
	{
...

		{
			...
			if (!item.IsSuccess() || !p_pSettingsList->Append(&item)) {
				g_Log(logLevelError, "X265 Plugin 10Bit :: Failed to add a separator entry");
				return errFail;
			}
		}
...

		{
			...
			if (!item.IsSuccess() || !p_pSettingsList->Append(&item)) {
				g_Log(logLevelError, "X265 Plugin 10Bit :: Failed to populate the button entry");
				return errFail;
			}
		}
...
private:
	...
	StatusCode RenderGeneral(HostListRef* p_pSettingsList)
	{
		if (0) {
			...
			if (!item.IsSuccess() || !p_pSettingsList->Append(&item)) {
				g_Log(logLevelError, "X265 Plugin 10Bit :: Failed to populate general label entry");
				return errFail;
			}
		}
		...
		if (m_CommonProps.GetContainer().size() >= 32) {
			...
			if (!item.IsSuccess() || !p_pSettingsList->Append(&item)) {
				g_Log(logLevelError, "X265 Plugin 10Bit :: Failed to populate encoder preset UI entry");
				assert(false);
				return errFail;
			}
		}
		...
		// Tune combobox
		{
			...
			if (!item.IsSuccess() || !p_pSettingsList->Append(&item)) {
				g_Log(logLevelError, "X265 Plugin 10Bit :: Failed to populate tune UI entry");
				return errFail;
			}
		}
		...
		// Profile combobox
		{
			...
			if (!item.IsSuccess() || !p_pSettingsList->Append(&item)) {
				g_Log(logLevelError, "X265 Plugin 10Bit :: Failed to populate profile UI entry");
				return errFail;
			}
		}
	...
	StatusCode RenderQuality(HostListRef* p_pSettingsList)
	{
		if (0) {
			...
			if (!item.IsSuccess() || !p_pSettingsList->Append(&item)) {
				g_Log(logLevelError, "X265 Plugin 10Bit :: Failed to populate quality label entry");
				return errFail;
			}
		}

		{
			...
			item.SetTriggersUpdate(true);
			if (!item.IsSuccess() || !p_pSettingsList->Append(&item)) {
				g_Log(logLevelError, "X265 Plugin 10Bit :: Failed to populate passes UI entry");
				return errFail;
			}
		}
		...
		if (m_NumPasses < 2) {
			...
			if (!item.IsSuccess() || !p_pSettingsList->Append(&item)) {
				g_Log(logLevelError, "X265 Plugin 10Bit :: Failed to populate quality UI entry");
				return errFail;
			}
		}

		{
			...
			if (!item.IsSuccess() || !p_pSettingsList->Append(&item)) {
				g_Log(logLevelError, "X265 Plugin 10Bit :: Failed to populate qp slider UI entry");
				return errFail;
			}
		}

		{
			...
			if (!item.IsSuccess() || !p_pSettingsList->Append(&item)) {
				g_Log(logLevelError, "X265 Plugin 10Bit :: Failed to populate bitrate slider UI entry");
				return errFail;
			}
		}
...
StatusCode X265Encoder10b::s_GetEncoderSettings(HostPropertyCollectionRef* p_pValues, HostListRef* p_pSettingsList)
{
...
StatusCode X265Encoder10b::s_RegisterCodecs(HostListRef* p_pList)
{
	...
	codecInfo.SetProperty(pIOPropUUID, propTypeUInt8, X265Encoder10b::s_UUID, 16);
...
void X265Encoder10b::DoFlush()
{

	g_Log(logLevelInfo, "X265 Plugin 10Bit :: DoFlush");
...
StatusCode X265Encoder10b::DoInit(HostPropertyCollectionRef* p_pProps)
{
...
void X265Encoder10b::SetupContext(bool p_IsFinalPass)
{

	std::string logMessagePrefix = "X265 Plugin 10Bit :: SetupContext :: ";
	std::ostringstream logMessage;
	...
	if (x265_param_default_preset(m_pParam, m_pSettings->GetEncPreset(), m_pSettings->GetTune()) != 0) {
		g_Log(logLevelInfo, "X265 Plugin 10Bit :: SetupContext :: setting x265 param default presets failed");
		m_Error = errFail;
		return;
	}
...
StatusCode X265Encoder10b::DoOpen(HostBufferRef* p_pBuff)
{

	std::string logMessagePrefix = "X265 Plugin 10Bit :: DoOpen :: ";
	std::ostringstream logMessage;
	...
	if (hdrBytes > 0) {
		...
		for (int i = 0; i < numNals; i++) {
			...
			if (pNals[i].type == NAL_UNIT_PREFIX_SEI) {
				g_Log(logLevelInfo, "X265 Plugin 10Bit :: DoOpen :: continue");
				continue;
			}
...
StatusCode X265Encoder10b::DoProcess(HostBufferRef* p_pBuff)
{
	std::string logMessagePrefix = "X265 Plugin 10Bit :: DoProcess :: ";
	std::ostringstream logMessage;
	...
	if (p_pBuff == NULL || !p_pBuff->IsValid()) {
		...
		g_Log(logLevelInfo, "X265 Plugin 10Bit :: DoProcess :: trying to flush");
		...
		if (!p_pBuff->LockBuffer(&pBuf, &bufSize)) {
			g_Log(logLevelError, "X265 Plugin 10Bit :: Failed to lock the buffer");
			return errFail;
		}
		...
		if (!p_pBuff->GetUINT32(pIOPropWidth, width) || !p_pBuff->GetUINT32(pIOPropHeight, height)) {
			g_Log(logLevelError, "X265 Plugin 10Bit :: Width/Height not set when encoding the frame");
			return errNoParam;
		}

		if (!p_pBuff->GetINT64(pIOPropPTS, pts)) {
			g_Log(logLevelError, "X265 Plugin 10Bit :: PTS not set when encoding the frame");
			return errNoParam;
		}
		...
		// clrNV12 (but coming in as 16-bit?)  > I420 (10-bit)

		uint8_t* pSrc = reinterpret_cast<uint8_t*>(const_cast<char*>(pBuf));

		int iPixelBytes = m_pParam->sourceBitDepth > 8 ? 2 : 1;
		...
		uint8_t* uvSrc = pSrc;
		uvSrc += (ySize * iPixelBytes);

		for (int i = 0; i < (ySize / 4) * 2 * iPixelBytes; i += 4) {

			uPlane.push_back(uvSrc[0]);
			uPlane.push_back(uvSrc[1]);
			vPlane.push_back(uvSrc[2]);
			vPlane.push_back(uvSrc[3]);
