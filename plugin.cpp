...
static const uint8_t pMyUUID[] = { 0x5a, 0x43, 0xce, 0x60, 0x45, 0x11, 0x4f, 0x58, 0x87, 0xde, 0xf3, 0x02, 0x80, 0x1e, 0x7b, 0xbd };
...
{
    if (memcmp(p_pUUID, X265Encoder10b::s_UUID, 16) == 0)
    {
        *p_ppObj = new X265Encoder10b();
        return errNone;
    }
...
{
    StatusCode err = X265Encoder10b::s_RegisterCodecs(p_pList);
    if (err != errNone)
    {
        return err;
    }
...
StatusCode g_GetEncoderSettings(unsigned char* p_pUUID, HostPropertyCollectionRef* p_pValues, HostListRef* p_pSettingsList)
{
    if (memcmp(p_pUUID, X265Encoder10b::s_UUID, 16) == 0)
    {
        return X265Encoder10b::s_GetEncoderSettings(p_pValues, p_pSettingsList);
    }
